...
void AntBms::on_status_data_(const std::vector<uint8_t> &data) {
  ...
  float current = ((int32_t) ant_get_32bit(70)) / 1 * -1 * 0.1f;
  ...
  if (this->supports_new_commands_) {
    this->publish_state_(this->power_sensor_,  total_voltage * current * 1 * -1);
  } else {
    this->publish_state_(this->power_sensor_,  (float) (int32_t) ant_get_32bit(111)  / 1 * -1);
